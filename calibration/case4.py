...
class CrystalPlasticity(Problem):
    def custom_init(self, quat, cell_ori_inds):
        ...
        slip_resistance_gp = self.gss_initial*onp.ones((len(self.fes[0].cells), self.fes[0].num_quads, self.num_slip_sys))
        ## Hu: (1en(cells), num_quads, 12) * 60.8
        ...
        gss_a_gp = self.gss_a0*onp.ones((len(self.fes[0].cells), self.fes[0].num_quads))
        h_gp = self.h0*onp.ones((len(self.fes[0].cells), self.fes[0].num_quads))
        t_sat_gp = self.t_sat0*onp.ones((len(self.fes[0].cells), self.fes[0].num_quads))
        xm_gp = self.xm0*onp.ones((len(self.fes[0].cells), self.fes[0].num_quads))
        r_gp = self.r0*onp.ones((len(self.fes[0].cells), self.fes[0].num_quads))
    ...
    def get_maps(self):
        ...
        def get_partial_tensor_map(Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r):
            _, unflatten_fn = jax.flatten_util.ravel_pytree(Fp_inv_old)
            _, unflatten_fn_params = jax.flatten_util.ravel_pytree([Fp_inv_old, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r])
    
            def first_PK_stress(u_grad):
                x, _ = jax.flatten_util.ravel_pytree([u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r])
                y = newton_solver(x)
                S = unflatten_fn(y)
                _, _, _, Fe, F = helper(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r, S)
            ...
            def update_int_vars(u_grad):
                ...
                y = newton_solver(x)
                S = unflatten_fn(y)
                Fp_inv_new, slip_resistance_new, slip_new, Fe, F = helper(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r, S)
                return Fp_inv_new, slip_resistance_new, slip_new, rot_mat, gss_a, h, t_sat, xm, r

            def helper(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r, S):
                ## Hu: Assembling q_ab matrix here
            ...
            def implicit_residual(x, y):
                u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r = unflatten_fn_params(x)
                S = unflatten_fn(y)
                _, _, _, Fe, _ = helper(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r, S)
            ...
            def newton_solver(x):
                ...
                def body_fun(state):
                    ...
                    ## Hu: For matrices that are near-square, jacfwd probably has an edge over jacrev
        ...
        def tensor_map(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r):
            first_PK_stress, _ = get_partial_tensor_map(Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r)
            return first_PK_stress(u_grad)

        def update_int_vars_map(u_grad, Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r):
            _, update_int_vars = get_partial_tensor_map(Fp_inv_old, slip_resistance_old, slip_old, rot_mat, gss_a, h, t_sat, xm, r)
    ...
    def update_int_vars_gp(self, sol, params):
        ...
        checkpointed_update_int_vars_map = jax.checkpoint(update_int_vars_map)
        # vmap_update_int_vars_map = jax.jit(jax.vmap(jax.vmap(checkpointed_update_int_vars_map)))
        cpu_device = jax.devices("cpu")[0]
        vmap_update_int_vars_map = jax.jit(jax.vmap(jax.vmap(checkpointed_update_int_vars_map)), device=cpu_device)
        # (num_cells, 1, num_nodes, vec, 1) * (num_cells, num_quads, num_nodes, 1, dim) -> (num_cells, num_quads, num_nodes, vec, dim) 
        u_grads = np.take(sol, self.fes[0].cells, axis=0)[:, None, :, :, None] * self.fes[0].shape_grads[:, :, :, None, :] 
        u_grads = np.sum(u_grads, axis=2) # (num_cells, num_quads, vec, dim)
        Fp_inv_gp, slip_resistance_gp, slip_gp, rot_mats_gp, gss_a_gp, h_gp, t_sat_gp, xm_gp, r_gp = \
            vmap_update_int_vars_map(u_grads, *params)
        # TODO
        return [Fp_inv_gp, slip_resistance_gp, slip_gp, rot_mats_gp, gss_a_gp, h_gp, t_sat_gp, xm_gp, r_gp]
    ...
    def inspect_interval_vars(self, params):
        ...
        """
        Fp_inv_gp, slip_resistance_gp, slip_gp, rot_mats_gp, gss_a_gp, h_gp, t_sat_gp, xm_gp, r_gp = params
