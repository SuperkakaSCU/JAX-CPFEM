...
## single crystal (FCC) case under tensile loadings
import jax
...
from applications.GPU.models_copper import CrystalPlasticity

os.environ["CUDA_VISIBLE_DEVICES"] = "2"

case_name = 'singlecrystal_copper'
...
def problem():
    ...
    meshio_mesh = meshio.read(os.path.join(neper_folder, f"mesh25.msh"))
    ...
    disps = np.linspace(0., 0.05*Lx, 51)
    ts = np.linspace(0., 0.5, 51)
    ...
    def corner3(point):
        flag_x = np.isclose(point[0], Lx, atol=1e-3)
        flag_y = np.isclose(point[1], 0., atol=1e-3)
        flag_z = np.isclose(point[2], 0., atol=1e-3)
        return np.logical_and(np.logical_and(flag_x, flag_y), flag_z)
...
if __name__ == "__main__":
    start_time_copper = time.time()
    problem()
    end_time_copper = time.time()
    run_time_copper = end_time_copper - start_time_copper
    print("Simulation time:", run_time_copper)
    print("This is for copper (FCC)")
